        ...
        fetch(`/games/${gameId}/projects/${projectId}/progress`, {
            ...
            .then((data) => {
                const initialComplexity =
                    parseInt(card.dataset.initialComplexity) || 100;
                const remainingComplexity = data.complex;
                const percentage = Math.max(
                    0,
                    Math.min(
                        100,
                        ((initialComplexity - remainingComplexity) /
                            initialComplexity) *
                            100,
                    ),
                );
                ...
                const progressPercentage = card.querySelector(
                    ".progress-percentage",
                );
...
function updateProjectProgressGlobal() {
    ...
    if (gameCard) {
        ...
        fetch(`/games/${gameId}/update-all-projects`, {
            ...
            .catch((error) =>
                console.error(
                    "Errore aggiornamento progetti globalmente:",
                    error,
                ),
            );
    }
            ...
            for (const gameId in data.games) {
                ...
                if (gameCard) {
                    ...
                    if (patrSpan && gameData.patrimonio !== undefined) {
                        patrSpan.textContent =
                            "€ " +
                            new Intl.NumberFormat("it-IT").format(
                                Math.round(gameData.patrimonio),
                            );
                        // puoi anche aggiornare la classe per colore verde/rosso, se vuoi
                        patrSpan.classList.toggle(
                            "text-green-500",
                            gameData.patrimonio > 0,
                        );
                        patrSpan.classList.toggle(
                            "text-red-500",
                            gameData.patrimonio <= 0,
                    ...
                    const actionDiv = gameCard.querySelector(
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const gamesInProgress = document.querySelectorAll(
        '[data-game-id][data-game-state="in_progress"]',
    );
